...
let hasAuthConfigurationError = false;
let hasAuthInitStarted = false;
...
const App = () => {
  ...
  const [questions, setQuestions] = useState([
    {
      id: 1,
      type: 'multiple',
      text: 'Пресметај ја плоштината на круг со радиус $r = 5cm$ ако $\\pi \\approx 3.14$?',
      options: ['$78.5cm^2$', '$31.4cm^2$', '$25cm^2$'],
      correct: 0,
      columns: 2,
      points: 5,
      subNum: '',
      difficulty: 'medium',
      showImage: true,
      imageUrl: 'https://img.freepik.com/free-vector/math-concept-illustration_114360-3949.jpg',
    },
  ]);
  ...
  useEffect(() => {
    const initAuth = async () => {
      if (hasAuthConfigurationError || hasAuthInitStarted) return;
      hasAuthInitStarted = true;
      try {
        await signInAnonymously(auth);
      } catch (err) {
        // In local/dev setups without Firebase Auth fully configured,
        // allow the editor to continue in offline-ish mode.
        if (err?.code === 'auth/configuration-not-found') {
          hasAuthConfigurationError = true;
          if (import.meta.env.DEV) {
            console.warn(
              '[auth] Anonymous auth is not configured for this Firebase project. Continuing without cloud sync.'
            );
          }
          return;
        }
  ...
  useEffect(() => {
    ...
    const unsubBank = onSnapshot(
      collection(db, 'artifacts', appId, 'users', user.uid, 'question_bank'),
      (s) => {
        setQuestionBank(s.docs.map((d) => ({ id: d.id, ...d.data() })));
      }
    );
    const unsubTests = onSnapshot(
      collection(db, 'artifacts', appId, 'users', user.uid, 'my_tests'),
      (s) => {
        ...
        <>
          ...
          <div className="fixed inset-0 z-[150] flex items-end justify-center pb-20 pointer-events-none px-4">
            <div className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-[0_30px_60px_-12px_rgba(0,0,0,0.5)] relative pointer-events-auto border border-slate-100 animate-in fade-in slide-in-from-bottom-5 duration-500">
              ...
              <div className="bg-indigo-600 w-16 h-16 rounded-2xl flex items-center justify-center text-white mb-6 shadow-lg shadow-indigo-100">
                <Target size={32} />
              </div>
              <h2 className="text-3xl font-black mb-3 tracking-tighter text-slate-900 leading-none">
        ...
        >
          <div className={`${sidebarOpen ? 'block' : 'hidden'} space-y-10`}>
                    ...
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition flex gap-1">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          deleteDoc(
                            doc(db, 'artifacts', appId, 'users', user.uid, 'question_bank', bq.id)
                          );
                        }}
            ...
            <div id="tests-tab" className="mt-10">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                <BookOpen size={12} /> Мои Тестови ({myTests.length})
              </h3>
                ...
                {myTests.map((t) => (
                  ...
                  >
                    ...
                    <p className="text-[8px] font-bold text-slate-400 mt-1 uppercase">
                      {t.testInfo.grade} одделение • {t.questions.length} прашања
                    </p>
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition flex gap-1">
                      <button
                        onClick={(e) => {
        ...
        <main className="flex-1 p-12 bg-slate-50/50 flex flex-col items-center">
          ...
          >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-2xl border border-slate-100 bg-slate-50 text-slate-600 hover:bg-white transition"
            >
              <Shuffle size={14} />
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[300] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl p-10 animate-in zoom-in-95 duration-300">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-2">
                    Копирајте го JSON кодот од AI подолу
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex gap-4">
              <button
                onClick={() => setShowPasteModal(false)}
                className="flex-1 py-5 rounded-[1.5rem] font-black text-slate-400 hover:bg-slate-50 transition uppercase tracking-widest text-xs"
              >
                Откажи
              </button>
              <button
                ...
                className="flex-[2] py-5 bg-indigo-600 text-white rounded-[1.5rem] font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition uppercase tracking-widest text-xs"
              >
                Увези задачи
              </button>
            </div>
...
if (!mountNode) {
  throw new Error('Root mount node #root was not found.');
}

// Vite HMR can re-evaluate this module; reuse the same root instance.
const ROOT_INSTANCE_KEY = '__makedotest_root__';
const root = mountNode[ROOT_INSTANCE_KEY] ?? ReactDOM.createRoot(mountNode);
